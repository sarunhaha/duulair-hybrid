...
export default function GroupRegistrationPage() {
  ...
  useEffect(() => {
    if (!isGroup || !groupId) {
      setContextError('กรุณาเปิดจากกลุ่ม LINE');
    }
  }, [isGroup, groupId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!groupId) {
      alert('ไม่พบ Group ID กรุณาเปิดจากกลุ่ม LINE');
      return;
    }
  ...
  // Show error if not in group context
  if (contextError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background">
        <Card className="w-full max-w-sm border-destructive/20">
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <h1 className="text-xl font-bold text-foreground">ไม่สามารถใช้งานได้</h1>
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        <Card className="bg-gradient-to-br from-primary to-primary/80 text-white border-none shadow-lg overflow-hidden relative mb-6">
          ...
          <CardContent className="pt-8 pb-8 text-center relative z-10">
            ...
            <h1 className="text-2xl font-bold mb-1">ลงทะเบียนกลุ่ม OONJAI</h1>
            <p className="text-white/90">กรุณากรอกข้อมูลเพื่อเริ่มใช้งานในกลุ่ม</p>
          </CardContent>
        </Card>
